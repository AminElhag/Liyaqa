...
const trendConfig = {
  up: { icon: TrendingUp, color: 'text-status-success', bg: 'bg-status-success-bg' },
  down: { icon: TrendingDown, color: 'text-status-error', bg: 'bg-status-error-bg' },
  neutral: { icon: Minus, color: 'text-muted-foreground', bg: 'bg-muted' },
}

export function StatCard({ label, value, change, trend = 'neutral', icon: Icon, loading, delay = 0 }: StatCardProps) {
  if (loading) {
    return (
