    ...
    >
        ...
        <ClientLifecycleFunnel
          ...
          onStageClick={(stage) => {
            const routes: Record<string, string> = {
              TRIAL: `/platform/clients?status=TRIAL`,
              ONBOARDING: `/platform/clients?status=ONBOARDING`,
              ACTIVE: `/platform/clients?status=ACTIVE`,
              AT_RISK: `/platform/health?risk=HIGH`,
              CHURNED: `/platform/clients?status=CHURNED`,
            };
      ...
      <motion.div variants={itemVariants} className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <HealthOverview
            ...
            onClientClick={(orgId) => navigate(`/platform/clients/${orgId}/health`)}
          />
        </div>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="grid gap-6 lg:grid-cols-2">
        <OnboardingMonitor
          ...
          onViewAll={() => navigate(`/platform/clients?status=ONBOARDING`)}
          onClientClick={(orgId) => navigate(`/platform/clients/${orgId}`)}
          isLoading={isLoading}
        />
        <AlertCenter
          alerts={alerts || []}
          ...
          onAlertClick={(alert) => navigate(`/platform/alerts?id=${alert.id}`)}
        />
      </motion.div>
      ...
      {hasActiveDunning && (
        <motion.div variants={itemVariants}>
          <DunningStatusWidget
            ...
            onSequenceClick={(id) => navigate(`/platform/dunning?id=${id}`)}
            isLoading={isLoading}
          />
        </motion.div>
      )}
