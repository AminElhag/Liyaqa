...
export function SalesDashboard({
  ...
  expiringSubscriptions: _expiringSubscriptions,
  isLoading,
}: SalesDashboardProps) {
    ...
    >
      ...
      >
        ...
        <div className={cn("flex gap-2", isRtl && "flex-row-reverse")}>
          <Link to="/platform/deals/new">
            <Button className="gap-2">
              <Plus className="h-4 w-4" />
              {texts.newDeal}
            </Button>
          </Link>
          <Link to="/platform/clients/new">
      ...
      <motion.div variants={cardVariants} className="grid gap-6 lg:grid-cols-2">
        <TrialFollowUpCard
          trials={trialClients}
          locale={locale}
          onViewAll={() => navigate(`/platform/clients?status=TRIAL`)}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <UpsellOpportunitiesCard
          ...
          locale={locale}
          onViewAll={() => navigate(`/platform/health?filter=usage`)}
          onClientClick={(orgId) => navigate(`/platform/clients/${orgId}`)}
        />
      </div>
