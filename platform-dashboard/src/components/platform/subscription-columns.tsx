...
import type { ClientSubscriptionSummary, Money, LocalizedText } from "@/types";
 ...
 */
function formatDateStr(dateString: string, locale: string): string {
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">
            {row.original.organizationName
              ? getLocalizedText(row.original.organizationName as LocalizedText, locale)
        ...
        <div className="flex items-center gap-2">
          ...
          <span>
            {row.original.planName
              ? getLocalizedText(row.original.planName as LocalizedText, locale)
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1 text-muted-foreground">
          ...
          <span>{formatDateStr(row.original.startDate, locale)}</span>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <span>{formatDateStr(row.original.endDate, locale)}</span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <span className="font-medium">
          {formatMoney(row.original.agreedPrice, locale)}
        </span>
      ),
    },
    {
