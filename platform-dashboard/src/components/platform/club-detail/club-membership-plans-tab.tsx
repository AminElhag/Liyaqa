...
import { useClubMembershipPlans } from "@/hooks/use-club-detail";
import type { ClubMembershipPlan, BillingPeriod } from "@/types";
...
export function ClubMembershipPlansTab({ clubId, locale }: ClubMembershipPlansTabProps) {
  ...
  const columns: ColumnDef<ClubMembershipPlan, unknown>[] = [
    {
      accessorKey: "name",
      header: texts.name,
      cell: ({ row }) => {
