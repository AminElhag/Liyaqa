...
import { usePTClass, useUpdatePTClass } from "@liyaqa/shared/queries/use-pt-sessions";
...
const texts = {
  en: {
    back: "Back to PT Class",
    title: "Edit PT Class",
    subtitle: "Update the personal training class details",
    // Form labels
    nameEn: "Name (English)",
    nameAr: "Name (Arabic)",
    ...
    optional: "Optional",
    // Loading
    loading: "Loading PT class...",
  },
  ar: {
    back: "العودة لفصل التدريب الشخصي",
    title: "تعديل فصل التدريب الشخصي",
    subtitle: "تحديث تفاصيل فصل التدريب الشخصي",
    // Form labels
    ...
    optional: "اختياري",
    // Loading
    loading: "جاري تحميل فصل التدريب الشخصي...",
  },
};
...
export default function EditPTClassPage() {
  ...
  // Fetch existing PT class
  const { data: ptClass, isLoading: classLoading } = usePTClass(id);
  ...
  const updatePTClass = useUpdatePTClass();
  ...
  useEffect(() => {
    if (ptClass && !initialized) {
      setNameEn(ptClass.name?.en ?? "");
      setNameAr(ptClass.name?.ar ?? "");
      setDescriptionEn(ptClass.description?.en ?? "");
      setDescriptionAr(ptClass.description?.ar ?? "");
      setTrainerId(ptClass.trainerId ?? "");
      setSessionType(ptClass.ptSessionType ?? "ONE_ON_ONE");
      setLocationType(ptClass.ptLocationType ?? "CLUB");
      setDuration(ptClass.durationMinutes ?? 60);
      setMaxCapacity(ptClass.capacity ?? 4);
      setMinCapacity(ptClass.minCapacity ?? 1);
      setPricingModel(ptClass.pricingModel ?? "PAY_PER_ENTRY");
      setDropInPrice(ptClass.dropInPrice?.amount);
      setTravelFee(ptClass.travelFee?.amount);
      setTaxRate(ptClass.taxRate ?? 15);
      setCategoryId(ptClass.categoryId ?? "");
      setInitialized(true);
    }
  }, [ptClass, initialized]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await updatePTClass.mutateAsync({
        id,
        data: {
          nameEn: nameEn.trim(),
      ...
      router.push(`/${locale}/pt-classes/${id}`);
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3">
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push(`/${locale}/pt-classes/${id}`)}
          >
            {t.cancel}
          </Button>
          <Button type="submit" disabled={updatePTClass.isPending}>
            <Dumbbell className="me-2 h-4 w-4" />
            {updatePTClass.isPending ? t.saving : t.save}
          </Button>
        </div>
      </form>
    </div>
