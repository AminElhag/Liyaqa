...
import { ArrowLeft, ArrowRight, Ticket, Edit, Play, Pause, Trash2, Gift } from "lucide-react";
...
const SERVICE_TYPE_LABELS: Record<ServiceType, { en: string; ar: string }> = {
  GX: { en: "Group Exercise", ar: "تمارين جماعية" },
  PT: { en: "Personal Training", ar: "تدريب شخصي" },
  GOODS: { en: "Goods", ar: "منتجات" },
};
...
const texts = {
  en: {
    ...
    delete: "Delete",
    deleteConfirmTitle: "Delete Credit Pack",
    deleteConfirmDesc: "Are you sure you want to delete this credit pack? This action cannot be undone.",
    cancel: "Cancel",
    confirm: "Delete",
    packActivated: "Credit pack activated",
    packDeactivated: "Credit pack deactivated",
    packDeleted: "Credit pack deleted",
    packUpdated: "Credit pack updated",
    errorLoading: "Error loading credit pack",
    notFound: "Credit pack not found",
    details: "Details",
  ...
  ar: {
    ...
    deactivate: "تعطيل",
    delete: "حذف",
    deleteConfirmTitle: "حذف باقة الرصيد",
...
export default function CreditPackDetailPage() {
  ...
  const serviceTypeLabel = SERVICE_TYPE_LABELS[pack.serviceType];
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge
                  variant="secondary"
                  className={SERVICE_TYPE_COLORS[pack.serviceType] || ""}
                >
                  {locale === "ar" ? serviceTypeLabel.ar : serviceTypeLabel.en}
                </Badge>
              </div>
              {pack.description && (
      ...
      {isEditing ? (
        <>
          ...
          <ClassPackForm
            ...
            hideCategoryAllocations={pack.serviceType !== "GX"}
          />
        </>
      ) : (
        <>
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-6">
                ...
                {pack.serviceType === "GX" && (
                  <div>
                    <p className="text-sm text-muted-foreground">{t.allocationMode}</p>
                    <p className="text-lg font-semibold">
                      {pack.allocationMode === "PER_CATEGORY" ? t.perCategory : t.flat}
                    </p>
                  </div>
                )}
              </div>
          ...
          {/* Category Allocations (GX per-category only) */}
          {pack.serviceType === "GX" &&
            pack.allocationMode === "PER_CATEGORY" &&
            pack.categoryAllocations &&
            pack.categoryAllocations.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle>{t.categoryAllocations}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {pack.categoryAllocations.map((alloc) => (
                      <div
                        key={alloc.id}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="font-medium">
                            {alloc.categoryName ? (
                              <LocalizedText text={alloc.categoryName} />
                            ) : (
                              alloc.categoryId
                            )}
                          </span>
