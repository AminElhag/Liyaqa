...
import { Building2, Store, UserCog, CreditCard, Eye, EyeOff, Plus } from "lucide-react";
...
import type { BillingCycle, ClientPlanSummary } from "@liyaqa/shared/types/platform/client-plan";
import type { Deal, ConvertDealRequest } from "@liyaqa/shared/types/platform/deal";
...
type ConvertDealFormValues = z.infer<typeof convertDealSchema>;

interface ConvertDealFormProps {
  deal: Deal;
  onSubmit: (data: ConvertDealRequest) => void;
  onCancel: () => void;
...
const TAB_FIELDS: Record<string, (keyof ConvertDealFormValues)[]> = {
  organization: ["organizationNameEn", "organizationEmail", "organizationWebsite"],
  club: ["clubNameEn"],
  admin: ["adminEmail", "adminPassword", "adminDisplayNameEn"],
  subscription: ["clientPlanId"],
};

const TAB_ORDER = ["organization", "club", "admin", "subscription"];

export function ConvertDealForm({ deal, onSubmit, onCancel, isSubmitting }: ConvertDealFormProps) {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<ConvertDealFormValues>({
    resolver: zodResolver(convertDealSchema),
    defaultValues: {
      // Pre-populate from deal data
      organizationNameEn: deal.companyName || "",
      organizationNameAr: "",
      organizationTradeNameEn: "",
      organizationTradeNameAr: "",
      organizationType: "",
      organizationEmail: deal.contactEmail || "",
      organizationPhone: deal.contactPhone || "",
      organizationWebsite: "",
      vatRegistrationNumber: "",
      commercialRegistrationNumber: "",
      clubNameEn: deal.companyName ? `${deal.companyName} Club` : "",
      clubNameAr: "",
      clubDescriptionEn: "",
      clubDescriptionAr: "",
      adminEmail: deal.contactEmail || "",
      adminPassword: "",
      adminDisplayNameEn: deal.contactName || "",
      adminDisplayNameAr: "",
      createSubscription: !!deal.interestedPlanId,
      clientPlanId: deal.interestedPlanId || "",
      agreedPriceAmount: deal.estimatedValue?.amount || 0,
      agreedPriceCurrency: deal.estimatedValue?.currency || "SAR",
  ...
  // Navigate to first tab with errors on form validation failure
  const onInvalid = () => {
  ...
  const texts = {
    ...
    organizationDesc:
      locale === "ar"
        ? "سيتم إنشاء منظمة جديدة لهذا العميل"
        : "A new organization will be created for this client",
    ...
    submit: locale === "ar" ? "تحويل إلى عميل" : "Convert to Client",
    submitting: locale === "ar" ? "جاري التحويل..." : "Converting...",
  ...
  const handlePlanCreated = (plan: ClientPlanSummary) => {
    ...
    // Update agreed price to the plan's monthly price
    setValue("agreedPriceAmount", plan.monthlyPrice.amount);
    setValue("agreedPriceCurrency", plan.monthlyPrice.currency);
  };

  const handleFormSubmit = (data: ConvertDealFormValues) => {
    const request: ConvertDealRequest = {
