...
export default function MemberLoginPage() {
  ...
  // Localized texts
  const texts = {
    title: locale === "ar" ? "تسجيل دخول الأعضاء" : "Member Login",
    description:
      locale === "ar"
        ? "أدخل بيانات حسابك للوصول إلى بوابة الأعضاء"
        : "Enter your credentials to access the member portal",
    wrongRole:
      locale === "ar"
        ? "هذا الحساب غير مصرح له بالوصول إلى بوابة الأعضاء. يرجى استخدام تسجيل دخول الموظفين."
        : "This account cannot access the member portal. Please use the staff login.",
    staffLoginLink:
      locale === "ar"
        ? "هل أنت موظف؟ تسجيل دخول الموظفين"
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      // Validate role - only MEMBER can use this login
      if (user?.role !== "MEMBER") {
